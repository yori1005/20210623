...
$imagename = $_POST['imagename'];
    ...
    $sql = "UPDATE foodstuff SET food1 = :food1,food2 = :food2, food3 = :food3,food4 = :food4, food5 = :food5,food6 = :food6, food7 = :food7,food8 = :food8, food9 = :food9,food10 = :food10,
			...
			volfood9 = :volfood9,volfood10 = :volfood10,recipename = :recipename,id = :id WHERE recipename = '$recipename' AND id = '$id'";
...
for($i=1;$i<=10;$i++){
    ...
    if (!empty($_FILES['image']['name'])) {//ファイルが選択されていれば$imageにファイル名を代入
		...
		$res = glob("./foodimages/$imagename/");
        foreach($res as $f){
            if("$f" == "./foodimages/$imagename"){
                unlink($f);
            }
        }

		$sql = "UPDATE post SET id = :id,name = :name,recipename = :recipename,recipecomment = :recipecomment,calorie = :calorie,foodimage = :foodimage WHERE recipename = '$recipename' AND id = '$id'";
		$stmt = $dbh->prepare($sql);
		...
		$sql = "UPDATE post SET id = :id,name = :name,recipename = :recipename,recipecomment = :recipecomment,calorie = :calorie WHERE recipename = '$recipename'  AND id = '$id'";
		$stmt = $dbh->prepare($sql);
		$params = array(':id' => $id,':name' => $name,':recipename' => $recipename,':recipecomment' => $recipecomment,':calorie' => $calorie);
		$stmt->execute($params);
		}
